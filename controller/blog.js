...
class BlogCtl {
  ...
  async getDetails(ctx) {
    const { blogid, type = "select" } = ctx.request.query;
    if (!blogid) {
    ...
    if (type === "update") {
      ...
      const list = await BlogDetail.findOne({
        ...
        ctx.body = new SuccessModel({
            data: {
                cover: res2.dataValues,
                detail: list.dataValues
            }
        })
  ...
  async fixblog(ctx) {
    ...
    let blogtitle =
      r1[0] > 0 ? "博客内容更新成功" : "暂无变化 | 也有可能SQL错误";
    let detailtitle =
      r2[0] > 0 ? "博客详情更新成功" : "暂无变化 | 也有可能SQL错误";
    ctx.body = new SuccessModel({
      data: [blogtitle, detailtitle],
      message: "更新状态",
    });
  }
  async addblog(ctx) {
